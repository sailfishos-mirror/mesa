...
static void rvce_destroy(struct pipe_video_codec *encoder)
{
   ...
   if (enc->stream_handle) {
      struct rvce_fb_buffer fb = {
         .res = si_resource(pipe_buffer_create(enc->screen, 0, PIPE_USAGE_STAGING, FEEDBACK_SIZE)),
      };
      enc->fb = &fb;
      session(enc);
      destroy(enc);
                                  ...
                                  struct pipe_resource *destination, void **fb)
{
   ...
   *fb = enc->fb = CALLOC_STRUCT(rvce_fb_buffer);
   enc->fb->res = si_resource(pipe_buffer_create(enc->screen, 0, PIPE_USAGE_STAGING, FEEDBACK_SIZE));
   if (!enc->fb->res) {
