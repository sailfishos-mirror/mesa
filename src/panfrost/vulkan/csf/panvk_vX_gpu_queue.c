...
finish_render_desc_ringbuf(struct panvk_gpu_queue *queue)
{
   ...
   if (ringbuf->addr.dev) {
      ...
      panvk_as_free(dev, &dev->as.heap, ringbuf->addr.dev, ringbuf->size * 2);
   }
...
init_render_desc_ringbuf(struct panvk_gpu_queue *queue)
{
   ...
   dev_addr =
      panvk_as_alloc(dev, &dev->as.heap, ringbuf->size * 2, ringbuf->size * 2);
   ...
   if (ret) {
      panvk_as_free(dev, &dev->as.heap, dev_addr, ringbuf->size * 2);
      return panvk_errorf(dev, VK_ERROR_OUT_OF_DEVICE_MEMORY,
                        ...
                        enum panvk_subqueue_id subqueue)
{
   ...
   if (subq->tracebuf.addr.dev) {
      ...
      panvk_as_free(dev, &dev->as.heap, subq->tracebuf.addr.dev,
                      ...
                      enum panvk_subqueue_id subqueue)
{
   ...
   uint64_t pgsize = panvk_get_gpu_page_size(dev);
   dev_addr =
      panvk_as_alloc(dev, &dev->as.heap, subq->tracebuf.size + pgsize, pgsize);
   ...
   if (ret) {
      panvk_as_free(dev, &dev->as.heap, dev_addr, subq->tracebuf.size + pgsize);
      return panvk_errorf(dev, VK_ERROR_OUT_OF_DEVICE_MEMORY,
                          "Failed to GPU map ringbuf BO");
   }
