...
fn remat_src(
    ...
    ssa_const: &FxHashMap<SSAValue, u32>,
    src: &mut Src,
    is_sr_src: bool,
) {
...
fn remat_copies(
    ...
    for (i, src) in op.srcs_mut().iter_mut().enumerate() {
        ...
        remat_src(b, ssa_const, src, is_sr_src);
    }
}
...
impl Shader<'_> {
    pub fn remat_constants(&mut self) {
        ...
        self.map_instrs(|mut instr, ssa_alloc| {
            // We have to handle these separately
            if matches!(&instr.op, Op::PhiSrc(_) | Op::RegOut(_)) {
                return [instr].into();
            }
            ...
            remat_copies(&mut b, &ssa_const, &mut instr.op);
            b.push_instr(instr);
            b.into_mapped()
        });
        ...
        for block in self.blocks.iter_mut() {
            ...
            for instr in block.iter_postlude_mut() {
                remat_copies(&mut b, &ssa_const, &mut instr.op);
            }
