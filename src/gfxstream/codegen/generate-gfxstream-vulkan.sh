...
export MESA_DIR="$(realpath "$SCRIPT_DIR/../../..")"
if [ -z "$1" ];
then
...
if [ -d "$GFXSTREAM_DIR" ]; then
    python3 "$GEN_VK" -registry "$VK_XML" -registryGfxstream "$CUSTOM_XML" cereal -o "$GFXSTREAM_OUTPUT_DIR"
else
    echo "Gfxstream host directory not found, skipping host code generation."
fi

export CEREAL_VARIANT=guest
export GFXSTREAM_GUEST_ENCODER_DIR="$MESA_DIR/src/gfxstream/guest/vulkan_enc/autogenerated"
...
if [ -d "$GFXSTREAM_HOST_DECODER_DIR" ]; then
